...
async function runScraperInExpress() {
  ...
  for (const source of enabledSources) {
    try {
      ...
      // Public trust rule:
      // Never fabricate opportunities when a source is silent, offline, blocked, or has no matching real opportunity links.
      if (itemsToProcess.length === 0) {
        stats.errors.push(`${source.name}: no real matching opportunity links found`);
        const currentSrcObj = db.sources.find((s: any) => s.id === source.id);
        if (currentSrcObj) {
          currentSrcObj.last_checked = runTimestamp;
          currentSrcObj.error_status = "No real matching opportunity links found";
        }
        continue;
      }
