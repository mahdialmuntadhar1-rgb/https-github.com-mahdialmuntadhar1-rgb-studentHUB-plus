...
function requireAdminBearer(req: express.Request, res: express.Response, next: express.NextFunction) {
  const header = req.header("authorization") || "";
  if (!header.startsWith("Bearer ") || header.slice("Bearer ".length).trim().length < 10) {
    res.status(401).json({ error: "Unauthorized", message: "Admin Bearer token is required." });
    return;
  }
  const expected = process.env.ADMIN_JWT || process.env.JAMIAATI_ADMIN_JWT;
  if (expected && header !== `Bearer ${expected}`) {
    res.status(403).json({ error: "Forbidden", message: "Admin token is invalid or expired." });
    return;
  }
  next();
}
...
function sourceCategoryScope(source: any): string {
  return String(source.category_scope || source.type || "mixed");
}

function categoryFromScope(scope: string): string | null {
  switch (scope) {
    case "jobs": return "job";
    case "scholarships": return "scholarship";
    case "internships": return "internship";
    case "trainings": return "training";
    case "events": return "event";
    case "fellowships": return "fellowship";
...
async function runScraperInExpress() {
  ...
  for (const source of enabledSources) {
    try {
      ...
      const sourceStartNew = stats.itemsNew;
      const sourceStartDuplicate = stats.itemsDuplicate;
      ...
      for (const item of itemsToProcess) {
        ...
        let descAR = "";
        let descKU = "";
        let category = categoryFromScope(sourceCategoryScope(source)) || "job";
        let gov = "all";
...
Respond strictly with a JSON object inside <json>...</json> xml tags following these keys exactly:
{
  ...
  "contentKU": "Clean Kurdish summary of terms & qualifications",
              "category": "Choose exactly one: 'job', 'internship', 'scholarship', 'training', 'event', 'volunteering', 'fellowship', 'competition', 'announcement', 'registration', 'exam'",
...
Output only valid JSON container!`;
            ...
            if (jsonMatch) {
              ...
              category = categoryFromScope(sourceCategoryScope(source)) || parsed.category || category;
        ...
        if (!titleAR) {
          ...
          else if (lowerText.includes("fellow")) category = "fellowship";
          else if (lowerText.includes("event") || lowerText.includes("meetup")) category = "event";
          else if (lowerText.includes("registration") || lowerText.includes("admission")) category = "registration";
          else if (lowerText.includes("exam")) category = "exam";
          else category = "job";
        ...
        category = categoryFromScope(sourceCategoryScope(source)) || category;
        if (!publicCategories.has(category)) category = "announcement";

        // Add pending scraped candidate. Nothing is public until admin approval.
        const newOpp = {
      ...
      stats.sourceLogs.push({
        source_id: source.id,
        source_name: source.name,
        category_scope: sourceCategoryScope(source),
        found_count: itemsToProcess.length,
        candidate_count: stats.itemsNew - sourceStartNew,
        duplicate_count: stats.itemsDuplicate - sourceStartDuplicate,
        error: null
      });
...
app.get("/api/opportunities", (req, res) => {
  const db = ensureDBShape(readDB());
  const approvedOnly = db.opportunities.filter((o: any) => o.status === "approved");
  res.json(approvedOnly);
});

// Opportunity automation admin API. These endpoints require an admin Bearer JWT.
app.get("/api/opportunity-automation/status", requireAdminBearer, (req, res) => {
  const db = ensureDBShape(readDB());
  const byStatus = db.opportunities.reduce((acc: Record<string, number>, item: any) => {
  ...
  res.json({
    sources: {
      ...
      active: db.sources.filter((source: any) => source.enabled !== false).length
    },
    candidates: {
    ...
    lastRun: db.logs[0] || null
  });
});

app.get("/api/opportunity-automation/sources", requireAdminBearer, (req, res) => {
  const db = ensureDBShape(readDB());
  res.json({ sources: db.sources, total: db.sources.length });
});

app.get("/api/opportunity-automation/candidates", requireAdminBearer, (req, res) => {
  const db = ensureDBShape(readDB());
  const status = String(req.query.status || "pending_review");
  const category = req.query.category ? String(req.query.category) : "";
  const candidates = db.opportunities
    .filter((item: any) => (item.status || "pending_review") === status)
...
app.get("/api/opportunity-automation/logs", requireAdminBearer, (req, res) => {
  const db = ensureDBShape(readDB());
  res.json(db.logs);
});

app.post("/api/opportunity-automation/run-now", requireAdminBearer, async (req, res) => {
  try {
    const stats = await runScraperInExpress();
    res.json({ success: true, published: false, stats });
  } catch (err: any) {
    res.status(500).json({ success: false, error: err.message });
  }
});

app.post("/api/opportunity-automation/run-source/:id", requireAdminBearer, async (req, res) => {
  const db = ensureDBShape(readDB());
  const source = db.sources.find((item: any) => item.id === req.params.id);
  if (!source) {
    res.status(404).json({ error: "Source not found" });
    return;
  }
  const originalEnabled = db.sources.map((item: any) => ({ id: item.id, enabled: item.enabled }));
  db.sources.forEach((item: any) => {
    item.enabled = item.id === source.id;
  });
  writeDB(db);
  try {
    const stats = await runScraperInExpress();
    const restored = ensureDBShape(readDB());
    restored.sources.forEach((item: any) => {
      const previous = originalEnabled.find((entry: any) => entry.id === item.id);
      if (previous) item.enabled = previous.enabled;
    });
    writeDB(restored);
    res.json({ success: true, sourceId: source.id, published: false, stats });
  } catch (err: any) {
    const restored = ensureDBShape(readDB());
    restored.sources.forEach((item: any) => {
      const previous = originalEnabled.find((entry: any) => entry.id === item.id);
...
app.post("/api/opportunity-automation/candidates/:id/approve", requireAdminBearer, (req, res) => {
  const db = ensureDBShape(readDB());
  const item = db.opportunities.find((candidate: any) => candidate.id === req.params.id);
  if (!item) {
  ...
  writeDB(db);
  res.json({ success: true, published: true, candidate: item });
});

app.post("/api/opportunity-automation/candidates/:id/reject", requireAdminBearer, (req, res) => {
  const db = ensureDBShape(readDB());
  const item = db.opportunities.find((candidate: any) => candidate.id === req.params.id);
  if (!item) {
    res.status(404).json({ error: "Candidate not found" });
  ...
  item.rejection_reason = req.body?.reason || "Rejected by admin";
  item.updated_at = new Date().toISOString();
  writeDB(db);
  res.json({ success: true, candidate: item });
});
