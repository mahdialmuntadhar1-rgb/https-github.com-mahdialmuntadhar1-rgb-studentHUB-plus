...
async function handleFriendRequests(request: Request, env: Env) {
  ...
  const incoming = await env.DB.prepare(
    `SELECT r.*, p.full_name AS requester_name, p.email AS requester_email, p.role AS requester_role
     FROM social_connection_requests r
...
async function handleCreateFriendRequest(request: Request, env: Env) {
  ...
  const existing = await env.DB.prepare(
    `SELECT * FROM social_connection_requests
  ...
  await env.DB.prepare(
    `INSERT INTO social_connection_requests (id, requester_id, recipient_id, status, message, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?)`
  ).bind(
    requestRow.id,
...
async function handleFriendRequestAction(request: Request, env: Env, requestId: string, action: string) {
  ...
  const row: any = await env.DB.prepare(
    "SELECT * FROM social_connection_requests WHERE id = ? LIMIT 1"
  ...
  if (action === "accept") {
    ...
    await env.DB.prepare("UPDATE social_connection_requests SET status = 'accepted', updated_at = ? WHERE id = ?")
  ...
  if (action === "decline") {
    ...
    await env.DB.prepare("UPDATE social_connection_requests SET status = 'declined', updated_at = ? WHERE id = ?")
      .bind(now, requestId).run();
  ...
  if (action === "cancel") {
    ...
    await env.DB.prepare("UPDATE social_connection_requests SET status = 'cancelled', updated_at = ? WHERE id = ?")
      .bind(now, requestId).run();
...
async function handleCreateMessageRequest(request: Request, env: Env) {
  ...
  const existing: any = await env.DB.prepare(
    `SELECT * FROM social_message_threads
     WHERE type = 'direct'
       AND status <> 'declined'
  ...
  if (!threadId) {
    ...
    await env.DB.prepare(
      "INSERT OR IGNORE INTO social_message_thread_members (thread_id, user_id, role, status, created_at) VALUES (?, ?, 'member', 'active', ?)"
  ...
  await env.DB.prepare("UPDATE social_message_threads SET last_message_at = ?, updated_at = ? WHERE id = ?")
...
async function handleMessageRequestAction(request: Request, env: Env, threadId: string, action: string) {
  ...
  const thread: any = await env.DB.prepare("SELECT * FROM social_message_threads WHERE id = ? LIMIT 1").bind(threadId).first();
  ...
  await env.DB.prepare("UPDATE social_message_threads SET status = ?, updated_at = ? WHERE id = ?")
...
async function handleMessageThreads(request: Request, env: Env) {
  ...
  const rows = await env.DB.prepare(
    `SELECT t.*,
     ...
     FROM social_message_threads t
     JOIN social_message_thread_members me ON me.thread_id = t.id AND me.user_id = ?
...
async function requireThreadMember(env: Env, threadId: string, userId: string) {
  const thread = await env.DB.prepare(
    `SELECT t.*
     FROM social_message_threads t
     JOIN social_message_thread_members m ON m.thread_id = t.id
...
async function handleThreadMessages(request: Request, env: Env, threadId: string) {
  ...
  await env.DB.prepare("UPDATE social_message_thread_members SET last_read_at = ? WHERE thread_id = ? AND user_id = ?")
