...
async function runMainScraper(env: Env, triggerType: string): Promise<any> {
  ...
  try {
    const { results } = await env.DB.prepare(
      "SELECT id, name, url, type, category_scope FROM sources WHERE enabled = 1"
    ).all();
    sourcesList = results || [];
  } catch (err) {
  ...
  for (const source of sourcesList) {
    try {
      ...
      const sourceStartNew = resultsStats.itemsNew;
      const sourceStartDuplicate = resultsStats.itemsDuplicate;
      ...
      for (const rawItem of rawOpportunities) {
        ...
        cleaned.category = classifyOpportunityCategory(cleaned.titleEN + " " + cleaned.contentEN, source.category_scope || source.type);
      ...
      await logScrapingActivity(env, {
        ...
        items_new: resultsStats.itemsNew - sourceStartNew,
        items_duplicate: resultsStats.itemsDuplicate - sourceStartDuplicate,
        errors: "",
        source_logs: JSON.stringify([{
          source_id: source.id,
          source_name: source.name,
          category_scope: source.category_scope || source.type || "mixed",
          found_count: rawOpportunities.length,
          candidate_count: resultsStats.itemsNew - sourceStartNew,
          duplicate_count: resultsStats.itemsDuplicate - sourceStartDuplicate,
          error: null
        }])
      });
...
function classifyOpportunityCategory(text: string, sourceType: string): string {
  ...
  const scoped = normalizeSourceCategory(sourceType);
  ...
  if (norm.includes("registration") || norm.includes("admission") || norm.includes("تسجيل") || norm.includes("قبول")) {
    return "registration";
  }
  if (norm.includes("exam") || norm.includes("امتحان") || norm.includes("اختبار")) {
    return "exam";
  }
...
function normalizeSourceCategory(sourceType: string): string | null {
  ...
  try {
    await env.DB.prepare(
      `INSERT INTO scraper_logs (
        id, timestamp, source_id, source_name, items_found, items_new, items_duplicate, errors, source_logs
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).bind(
      log.id,
      log.timestamp,
