...
export default function AppRestructured() {
  // Locale States
  ...
  // Main tab state (Opportunities vs Campus Life)
  const [mainTab, setMainTab] = useState<MainTabType>('opportunities');

  // Legacy navigation tab state (for profile, admin, etc.)
  ...
  }, [selectedGov, selectedUni, activeTab, mainTab]);
  ...
  // Synchronised dynamic fetcher of live highlights & opportunities
  useEffect(() => {
        ...
        if (oppsResponse.ok) {
          ...
          if (Array.isArray(list)) {
            const mappedOpps = list.map((item: any) => ({
              ...
              contentKU: item.contentKU || (item.description && /[\u0600-\u06FF]/.test(item.description) ? item.description : '') || 'تکایە سەرچاوەی سەرکی ببینە بۆ زانیاری.',
              author: {
  ...
  const handleAwardPoints = (qty: number) => {
    setUserProfile(prev => {
      ...
      const nextLevel = Math.floor(nextPoints / 150) + 1;
      return {
        ...prev,
        points: nextPoints,
  ...
  // Filter by main tab type
  const getFilteredByMainTab = (items: FeedItem[]) => {
    if (mainTab === 'opportunities') {
      // Show opportunities: jobs, internships, scholarships, training, events, announcements
      return items.filter(item => 
        ['job', 'internship', 'scholarship', 'training', 'event', 'announcement', 'full_time_job', 'part_time_job', 'volunteering', 'competition', 'graduation_project_support', 'fellowship', 'admission'].includes(item.type)
      );
    } else {
      // Show campus life: posts, photos, videos, stories, questions, campus_life, general
      return items.filter(item => 
        ['post', 'photo', 'video', 'story', 'question', 'anonymous_question', 'campus_life', 'general', 'club', 'study_group', 'local_service'].includes(item.type)
      );
    }
  };

  const finalFilteredItems = getFilteredByMainTab(filteredFeedItems);

  const handleShowAllLife = () => {
    setSelectedGov('all');
    setSelectedUni('all');
    setMainTab('campus-life');
  };
  ...
  // Story navigation handler
  const handleStoryNavigate = (tabId: 'home' | 'life' | 'ask' | 'future' | 'profile') => {
    if (tabId === 'future') {
      setMainTab('opportunities');
    } else if (tabId === 'life') {
      setMainTab('campus-life');
  ...
  // Render main content based on main tab
    ...
    if (finalFilteredItems.length === 0) {
      return (
        <EmptyState 
          language={language} 
          type={mainTab} 
          hasFilters={selectedGov !== 'all' || selectedUni !== 'all'}
        />
      );
    }
  ...
  // Legacy views for profile, admin, etc.
    ...
    <div id="jamiaati-portal" className="bg-[#FAF9FF] min-h-screen text-slate-800 antialiased font-sans" dir={isRTL ? 'rtl' : 'ltr'} lang={language}>
      <div className="w-full">
        ...
        <MainFilters
          ...
          selectedUni={selectedUni}
          setSelectedUni={setSelectedUni}
          governorates={IraqiGovernorates}
          institutions={institutions}
          institutionsLoading={institutionsLoading}
        />

        {/* 4. Story Highlights (Shortcuts) */}
        <div className="max-w-7xl mx-auto px-4 py-4">
          <StoryHighlights
            language={language}
            onNavigateTab={handleStoryNavigate}
          />
        </div>
        ...
        {/* 6. Main Content Area */}
        {activeTab === 'home' || activeTab === 'life' || activeTab === 'future' || activeTab === 'ask' ? (
        ...
        <nav 
          className="fixed bottom-0 left-0 right-0 bg-white/95 border-t border-slate-200 px-2 py-3 flex justify-around items-center backdrop-blur-md z-40 shadow-lg md:hidden"
        >
          <button
            onClick={() => { setMainTab('opportunities'); setActiveTab('home'); }}
            className={`flex flex-col items-center gap-1 py-1 px-2.5 rounded-2xl cursor-pointer transition-all duration-200 ${
              mainTab === 'opportunities' && activeTab === 'home'
          ...
          <button
            onClick={() => { setMainTab('campus-life'); setActiveTab('life'); }}
            className={`flex flex-col items-center gap-1 py-1 px-2.5 rounded-2xl cursor-pointer transition-all duration-200 ${
              mainTab === 'campus-life' && activeTab === 'life'
                ? 'text-orange-600 font-extrabold scale-105'
                : 'text-slate-400 hover:text-slate-700 hover:bg-slate-100/40'
            }`}
          >
        ...
        {/* Toast Notifications */}
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 w-full max-w-[340px] flex flex-col gap-2 pointer-events-none">
          <AnimatePresence>
            {toasts.map((toast) => (
              <motion.div
                key={toast.id}
