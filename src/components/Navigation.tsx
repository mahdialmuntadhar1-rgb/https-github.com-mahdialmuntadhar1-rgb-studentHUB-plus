    ...
    <header className="fixed top-0 left-0 right-0 bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl border-b border-gray-100 dark:border-gray-800 z-50 px-6 h-20 flex items-center justify-between">
      <div className="flex items-center gap-3">
        ...
        <div>
            ...
            <div className="text-[10px] font-bold text-primary/60 uppercase tracking-[0.2em]">Talaba Platform</div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className="p-2 bg-surface dark:bg-gray-800 text-secondary dark:text-white rounded-xl border border-gray-50 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700 transition-all active:scale-95"
          title="Change language"
        >
          <Globe size={16} strokeWidth={2.5} />
          <span className="text-[9px] font-black ml-1">{language === 'ar' ? 'ع' : language === 'en' ? 'EN' : 'ک'}</span>
        </button>
        <button
          onClick={toggleTheme}
