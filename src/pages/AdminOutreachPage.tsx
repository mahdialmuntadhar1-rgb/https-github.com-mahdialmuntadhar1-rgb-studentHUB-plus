...
export default function AdminOutreachPage() {
  ...
  const paginatedContacts = useMemo(() => {
    const start = (contactPage - 1) * contactLimit;
    return contacts.slice(start, start + contactLimit);
  }, [contacts, contactPage, contactLimit]);

  const paginatedRecipients = useMemo(() => {
    const start = (recipientPage - 1) * recipientLimit;
    return (selectedCampaign?.recipients || []).slice(start, start + recipientLimit);
  }, [selectedCampaign?.recipients, recipientPage, recipientLimit]);
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      setMessage(err?.message || 'Could not load outreach data. Check the admin session and Worker routes.');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!user) return <AdminAccessState title="Admin login required" body="سجل الدخول بحساب مدير للوصول إلى هذه الصفحة." />;
  if (!isAdmin) return <AdminAccessState title="Admin access only" body="هذه الصفحة مخصصة للمديرين فقط." />;
      ...
      <section className="mx-auto max-w-7xl">
        ...
        <div className="mb-5 rounded-[2rem] border border-cyan-400/20 bg-[#121B2E] p-6">
          ...
          <div className="mt-4 grid gap-2 md:grid-cols-3">
            <SafetyPill label="Dry run mode active" ok={Boolean(dashboard.config?.dryRun)} />
            <SafetyPill label="Email provider connected" ok={Boolean(dashboard.config?.providerConfigured)} />
            <SafetyPill label="SPF/DKIM/DMARC not verified manually yet" ok={false} warn />
          </div>
        </div>
        ...
        {tab === 'contacts' && (
          <div className="grid gap-4 lg:grid-cols-[320px_1fr]">
            ...
            <div className="overflow-hidden rounded-3xl border border-[#1F2E4D] bg-[#121B2E]">
              ...
              <OutreachPagination page={contactPage} limit={contactLimit} total={contacts.length} setPage={setContactPage} setLimit={setContactLimit} />
            </div>
          </div>
        )}
            ...
            <div className="rounded-3xl border border-[#1F2E4D] bg-[#121B2E] p-5">
              {!selectedCampaign ? (
                ...
                <>
                  ...
                  <OutreachPagination page={recipientPage} limit={recipientLimit} total={selectedCampaign.recipients.length} setPage={setRecipientPage} setLimit={setRecipientLimit} dark />
                </>
              )}
            </div>
          </div>
...
function OutreachPagination({
  ...
  const base = dark ? 'border-[#1F2E4D] bg-slate-950 text-slate-300' : 'border-[#1F2E4D] bg-[#121B2E] text-slate-300';
  return (
    <div className={`flex flex-wrap items-center justify-between gap-3 border-t p-3 text-xs font-black ${base}`}>
      <span>Page {page} of {totalPages} · Total {total}</span>
      <div className="flex gap-2">
        <select value={limit} onChange={(event) => { setLimit(Number(event.target.value)); setPage(1); }} className="rounded-xl bg-slate-900 px-3 py-2 outline-none">
          {[10, 20, 50, 100].map((item) => <option key={item}>{item}</option>)}
        </select>
        <button disabled={page <= 1} onClick={() => setPage(page - 1)} className="rounded-xl bg-slate-900 px-3 py-2 disabled:opacity-40">Previous</button>
        <button disabled={page >= totalPages} onClick={() => setPage(page + 1)} className="rounded-xl bg-slate-900 px-3 py-2 disabled:opacity-40">Next</button>
      </div>
    </div>
  );
}
...
function SafetyPill({ label, ok, warn = false }: { label: string; ok: boolean; warn?: boolean }) {
  const classes = warn
    ? 'border-amber-400/30 bg-amber-400/10 text-amber-200'
    : ok
    ? 'border-emerald-400/30 bg-emerald-400/10 text-emerald-200'
    : 'border-rose-400/30 bg-rose-400/10 text-rose-200';
  return <div className={`rounded-2xl border px-3 py-2 text-xs font-black ${classes}`}>{label}</div>;
}
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-slate-50 p-4" dir="rtl">
      <div className="max-w-md rounded-3xl bg-white p-8 text-center shadow-xl">
        ...
        <p className="mt-3 text-sm font-bold text-slate-500">{body}</p>
      </div>
    </main>
  );
}
