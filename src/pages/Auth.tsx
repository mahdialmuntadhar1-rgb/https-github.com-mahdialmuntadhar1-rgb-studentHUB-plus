...
export default function Auth() {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = isLogin 
        ...
        : { email: email.toLowerCase(), password, full_name: fullName };
      ...
      // Redirect to main app after successful login/registration
  ...
  const handleForgotPasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || data.message || 'Failed to send password reset request');
      }
      ...
      >
        <div className="p-8 md:p-12">
          <div className="flex flex-col items-center mb-10 text-center">
             {/* Talaba Branding - Three Lines */}
             <div className="mb-6 space-y-1">
               <p className="text-lg font-bold text-gray-600">طلبة</p>
               <h1 className="text-4xl font-black text-secondary">Talaba Portal</h1>
               <p className="text-lg font-bold text-gray-600">تەڵەبە</p>
             </div>

             {isForgotPassword ? (
               <>
                 <h2 className="text-2xl font-black text-secondary mb-2">استعادة الحساب</h2>
                 <p className="text-sm text-gray-500 font-bold">يرجى إدخال البريد الإلكتروني الخاص بك لتلقي رابط إعادة تعيين كلمة المرور</p>
               </>
             ) : (
               <>
                 <h2 className="text-2xl font-black text-secondary mb-2">
                    {isLogin ? 'أهلاً بك مجدداً' : 'انضم إلى Talaba'}
                 </h2>
              ...
              {resetSent ? (
                ...
                <form onSubmit={handleForgotPasswordSubmit} className="space-y-4">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-gray-700 uppercase tracking-widest mr-4">البريد الإلكتروني</label>
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full bg-white border-2 border-gray-300 py-4 pr-12 pl-4 rounded-2xl focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none font-bold transition-all text-left text-gray-800"
                        dir="ltr"
                      />
                    </div>
                  </div>
                  ...
                  <button
                    disabled={isLoading}
                    className="w-full bg-primary text-white py-5 rounded-3xl font-black text-lg shadow-xl shadow-primary/30 hover:bg-secondary hover:text-white transition-all active:scale-95 flex items-center justify-center gap-3 mt-8 disabled:opacity-50"
                  >
                    {isLoading ? (
            ...
            <form onSubmit={handleAuth} className="space-y-4">
              <AnimatePresence mode="wait">
                {!isLogin && (
                  <motion.div
                    ...
                    className="space-y-2"
                  >
                    <label className="text-[10px] font-black text-gray-700 uppercase tracking-widest mr-4">الاسم الكامل</label>
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full bg-white border-2 border-gray-300 py-4 pr-12 pl-4 rounded-2xl focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none font-bold transition-all text-gray-800"
                      />
                    </div>
                  </motion.div>
                )}
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between mr-4 ml-1">
                  <label className="text-[10px] font-black text-gray-700 uppercase tracking-widest">كلمة المرور</label>
                  {isLogin && (
                    <button 
                      type="button"
                      onClick={() => {
          ...
          {!isForgotPassword && (
            <div className="mt-8 text-center">
               <button 
                 ...
                 className="text-gray-600 font-bold hover:text-primary transition-colors text-sm"
               >
