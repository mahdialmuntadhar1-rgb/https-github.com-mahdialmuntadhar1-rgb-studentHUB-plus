...
export default function App() {
  ...
  const handleRoleToggle = () => {
    console.warn('Role switching is disabled in MVP. Admin role must come from backend.');
  };
      ...
      case 'profile':
        return (
          <ProfileView
            ...
            onNavigateAdmin={() => setActiveTab('profile')}
            onEditFeedItem={handleEditFeedItem}
            onDeleteFeedItem={handleDeleteFeedItem}
            isAdminMode={userProfile.role === 'staff'}
          />
      ...
      <div className="w-full max-w-md mx-auto min-h-screen bg-[#0B1020] shadow-2xl shadow-black/8 w-full relative flex flex-col border-x border-[#1F2E4D]">
        ...
        <AuthModal
          ...
          onAuthSuccess={(newUsername) => {
            ...
            try {
              authUser = JSON.parse(localStorage.getItem('jamiaati_auth_user') || 'null');
            } catch (e) {
              authUser = null;
            }
            ...
            setUserProfile(prev => ({
              ...prev,
              name: newUsername || authUser?.full_name || authUser?.name || 'Zara Al-Iraqi',
              role: (authUser?.role === 'admin' ? 'staff' : authUser?.role) || prev.role
            }));
            showToast(
